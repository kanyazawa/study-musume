...
import { GameTabs } from "@/components/stats/GameTabs"
...
import { AccuracyRadar } from "@/components/stats/AccuracyRadar"
import { CharacterBanner } from "@/components/stats/CharacterBanner"
import { BottomNav } from "@/components/stats/BottomNav"
...
export default function StatsPageV0() {
                                    ...
                                    >
                                        ...
                                        <div className="flex-1">
                                            <div className="flex items-center justify-between">
                                                <span className="text-sm font-bold text-[var(--foreground)]">{subject.name}</span>
                                                <span className="text-sm font-black" style={{ color: subject.color }}>{subject.score}%</span>
                                            </div>
                                            <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-[rgba(255,255,255,0.08)]">
                                                <div
                                                    className="h-full rounded-full transition-all duration-500"
                                                    style={{ width: `${subject.score}%`, backgroundColor: subject.color, boxShadow: `0 0 8px ${subject.color}60` }}
                                                />
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div className="relative overflow-hidden rounded-2xl border-2 border-[var(--game-gold)]/30 bg-gradient-to-r from-[rgba(255,214,66,0.15)] to-[rgba(255,107,166,0.1)] p-4">
                                ...
                                <div className="relative flex items-center gap-3">
                                    <div className="flex h-14 w-14 flex-col items-center justify-center rounded-2xl border-2 border-[var(--game-gold)]/40 bg-[var(--game-gold)]/10 shadow-[0_0_12px_rgba(255,214,66,0.2)]">
                                        <span className="text-2xl font-black text-[var(--game-gold)]">14</span>
                                        <span className="text-[8px] font-bold text-[var(--game-gold)]/80">DAYS</span>
                                    </div>
                                    <div>
                                        <p className="text-sm font-bold text-[var(--foreground)]">連続学習記録を更新中!</p>
                                        <p className="mt-0.5 text-[11px] text-[var(--muted-foreground)]">あと1日で新しいバッジを獲得!</p>
                                        <div className="mt-2 flex items-center gap-2">
                                            <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-[rgba(255,255,255,0.1)]">
                                                <div className="h-full rounded-full bg-gradient-to-r from-[var(--game-gold)] to-[var(--game-pink)] shadow-[0_0_6px_rgba(255,214,66,0.4)]" style={{ width: "93%" }} />
                                            </div>
                                            <span className="text-[10px] font-bold text-[var(--game-gold)]">14/15</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
