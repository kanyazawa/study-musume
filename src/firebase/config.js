...
// 現在ホストを authDomain に使う構成は配信先が /__/auth/* を正しくプロキシしている場合だけ安全。
// Cloudflare Pages やローカル IP 配信では OAuth ハンドラ先を壊しやすいため、明示 opt-in にする。
const authDomain = resolveFirebaseAuthDomain({
    configuredAuthDomain,
    runtimeHostname,
    useRuntimeHostForAuth: import.meta.env.VITE_FIREBASE_AUTH_USE_RUNTIME_HOST === "true"
});
